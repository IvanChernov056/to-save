#include "RadialWithSom.h"


RadialWithSom::RadialWithSom (int i_neuronsAmount, double i_inverseDispersion, double i_leakingDecayRate, 
                                double i_reduceRatio, double ridge) 
{
    ...
    d_reducer = std::make_unique<SomReducer>((int)(i_reduceRatio*i_neuronsAmount));

}

double RadialWithSom::test (const DataList& i_data, int i_skipLen, int i_learnLen, int i_generateLen, const std::string& i_name) {
...
double RadialWithSom::test2 (const DataList& i_data, int i_skipLen, int i_learnLen, int i_generateLen, const std::string& i_name) {
    learn(i_data, i_skipLen, i_learnLen);
    int etalonBias = i_skipLen + i_learnLen;
    DataList etalData(i_data.begin()+etalonBias, i_data.end());
...
void RadialWithSom::learn(const DataList& i_data, int i_skipLen, int i_learnLen) {
...
DataList RadialWithSom::generate(int i_generateLen) {
    DataList result;
...
DataList RadialWithSom::predict (const DataList& i_inp) {
    DataList result;
...
void RadialWithSom::skip(const DataList& i_feedback) {

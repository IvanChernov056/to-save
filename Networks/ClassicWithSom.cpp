...
#include "ClassicWithSom.h"


ClassicWithSom::ClassicWithSom (int neuronAmount, double inpVal, double timeConstant, double timeStep, double leakingDecayRate, 
        ...
        double i_zeroProbInInputWeight, double i_zeroProbInFeedbackWeight, double i_reduceRatio, double ridge) 
{
    ...
    d_reducer = std::make_unique<SomReducer>((int)(i_reduceRatio*neuronAmount));

    d_inputVec = OnesColumn(2);
    d_inputVec[0] = inpVal;
}

double ClassicWithSom::test (const DataList& i_data, int i_skipLen, int i_learnLen, int i_generateLen, const std::string& i_name) {
...
double ClassicWithSom::test2 (const DataList& i_data, int i_skipLen, int i_learnLen, int i_generateLen, const std::string& i_name) {
    learn(i_data, i_skipLen, i_learnLen);
    int etalonBias = i_skipLen + i_learnLen;
    DataList etalData(i_data.begin()+etalonBias, i_data.end());
...
void ClassicWithSom::learn(const DataList& i_data, int i_skipLen, int i_learnLen) {
...
DataList ClassicWithSom::generate(int i_generateLen) {
    DataList result;
...
DataList ClassicWithSom::predict (const DataList& i_inp) {
    DataList result;
...
void ClassicWithSom::skip(const DataVector& i_inp, const DataList& i_feedback) {
